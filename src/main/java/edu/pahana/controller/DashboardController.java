...
public class DashboardController extends HttpServlet {
    ...
    private Map<String, Object> getRevenueAnalytics() throws SQLException {
        ...
        BigDecimal currentMonthRevenue = allBills.stream()
            .filter(bill -> bill.getBillDate() != null && 
                          bill.getBillDate().isAfter(currentMonthStart) &&
                          "paid".equalsIgnoreCase(bill.getStatus()))
            .map(edu.pahana.model.Bill::getTotalAmount)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
        
        // Calculate last month revenue
        BigDecimal lastMonthRevenue = allBills.stream()
            .filter(bill -> bill.getBillDate() != null && 
                          bill.getBillDate().isAfter(lastMonthStart) &&
                          bill.getBillDate().isBefore(currentMonthStart) &&
                          "paid".equalsIgnoreCase(bill.getStatus()))
            .map(edu.pahana.model.Bill::getTotalAmount)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
        ...
        if (lastMonthRevenue.compareTo(BigDecimal.ZERO) > 0) {
            percentageChange = ((currentMonthRevenue.doubleValue() - lastMonthRevenue.doubleValue()) / lastMonthRevenue.doubleValue()) * 100;
        }
    ...
    private Map<String, Object> getQuickStats() throws SQLException {
        ...
        long newCustomers = allCustomers.stream()
            .filter(customer -> customer.getCreatedAt() != null && 
                              customer.getCreatedAt().toLocalDateTime().isAfter(currentMonthStart))
            .count();
    ...
    private Map<String, Object> getSystemStatus() {
        ...
        systemStatus.put("backupSystem", "Last: " + LocalDateTime.now().minusHours(2).format(DateTimeFormatter.ofPattern("MMM dd, HH:mm")));
